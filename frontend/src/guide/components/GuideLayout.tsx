...
export const GuideLayout: React.FC<GuideLayoutProps> = ({ children, breadcrumbs, stickyCta }) => {
  ...
  return (
    <div className="min-h-screen bg-surface flex justify-center w-full">
      <div className="w-full max-w-guide page-content py-6 sm:py-10 flex flex-col min-h-screen pb-24 sm:pb-10">
        ...
        <nav aria-label="Хлебные крошки" className="flex flex-wrap items-center gap-1 text-desc text-graphite-muted mb-8 min-w-0">
          <Link to="/" className="hover:text-accent flex items-center gap-1 text-safe">
          ...
          {breadcrumbs.map((item) => (
            <React.Fragment key={item.label}>
              ...
              {item.href ? (
                <Link to={item.href} className="hover:text-accent text-safe">
                  {item.label}
                </Link>
              ) : (
                <span className="text-graphite font-medium text-safe">{item.label}</span>
              )}
            </React.Fragment>
          ))}
        </nav>
      ...
      {stickyCta && (
        <div className="fixed bottom-0 left-0 right-0 z-40 sm:hidden bg-white/90 backdrop-blur-xl safe-bottom page-content py-3">
          {stickyCta}
        </div>
      )}
    </div>
