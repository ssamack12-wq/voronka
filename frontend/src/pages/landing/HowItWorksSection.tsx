          ...
          <React.Fragment key={step.num}>
            ...
            {i < STEPS.length - 1 && (
              <div className="flex items-center pt-5 shrink-0 px-1">
                <ChevronRight className="w-6 h-6 text-graphite-muted/40" strokeWidth={1.5} />
              </div>
            )}
          </React.Fragment>
        ))}
        ...
        {STEPS.map((step, i) => (
          <React.Fragment key={step.num}>
            <FadeIn delay={i * 80} className="w-full max-w-xs">
              <div className={`flex flex-col items-center text-center ${CARD_CLASS}`}>
            ...
            {i < STEPS.length - 1 && (
              <div className="py-3">
                <ArrowDown className="w-5 h-5 text-graphite-muted/40" strokeWidth={2} />
              </div>
            )}
          </React.Fragment>
        ))}
